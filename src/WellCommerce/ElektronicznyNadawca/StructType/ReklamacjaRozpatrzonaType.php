...
class ReklamacjaRozpatrzonaType extends AbstractStructBase
{
     ...
     * @uses \WellCommerce\ElektronicznyNadawca\EnumType\RozstrzygniecieType::valueIsValid()
     * @uses \WellCommerce\ElektronicznyNadawca\EnumType\RozstrzygniecieType::getValidValues()
    ...
    public function setRozstrzygniecie(?string $rozstrzygniecie = null): self
    {
        ...
        if (!\WellCommerce\ElektronicznyNadawca\EnumType\RozstrzygniecieType::valueIsValid($rozstrzygniecie)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \WellCommerce\ElektronicznyNadawca\EnumType\RozstrzygniecieType', is_array($rozstrzygniecie) ? implode(', ', $rozstrzygniecie) : var_export($rozstrzygniecie, true), implode(', ', \WellCommerce\ElektronicznyNadawca\EnumType\RozstrzygniecieType::getValidValues())), __LINE__);
        }
