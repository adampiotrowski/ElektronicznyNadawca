...
class ZwrotDokumentowBiznesowaType extends AbstractStructBase
{
    ...
    /**
     * Set rodzaj value
     * @uses \WellCommerce\ElektronicznyNadawca\EnumType\TerminZwrotDokumentowBiznesowaType::valueIsValid()
     * @uses \WellCommerce\ElektronicznyNadawca\EnumType\TerminZwrotDokumentowBiznesowaType::getValidValues()
     * @throws InvalidArgumentException
     * @param string $rodzaj
     * @return \WellCommerce\ElektronicznyNadawca\StructType\ZwrotDokumentowBiznesowaType
     */
    public function setRodzaj(string $rodzaj): self
    {
        // validation for constraint: enumeration
        if (!\WellCommerce\ElektronicznyNadawca\EnumType\TerminZwrotDokumentowBiznesowaType::valueIsValid($rodzaj)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \WellCommerce\ElektronicznyNadawca\EnumType\TerminZwrotDokumentowBiznesowaType', is_array($rodzaj) ? implode(', ', $rodzaj) : var_export($rodzaj, true), implode(', ', \WellCommerce\ElektronicznyNadawca\EnumType\TerminZwrotDokumentowBiznesowaType::getValidValues())), __LINE__);
        }
        $this->rodzaj = $rodzaj;
