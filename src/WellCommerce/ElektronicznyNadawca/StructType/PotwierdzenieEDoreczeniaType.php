...
class PotwierdzenieEDoreczeniaType extends AbstractStructBase
{
     ...
     * @param string $sposob
     * @return \WellCommerce\ElektronicznyNadawca\StructType\PotwierdzenieEDoreczeniaType
     */
    public function setSposob(string $sposob): self
    {
        // validation for constraint: enumeration
        if (!\WellCommerce\ElektronicznyNadawca\EnumType\ESposobPowiadomieniaType::valueIsValid($sposob)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \WellCommerce\ElektronicznyNadawca\EnumType\ESposobPowiadomieniaType', is_array($sposob) ? implode(', ', $sposob) : var_export($sposob, true), implode(', ', \WellCommerce\ElektronicznyNadawca\EnumType\ESposobPowiadomieniaType::getValidValues())), __LINE__);
        }
        $this->sposob = $sposob;
