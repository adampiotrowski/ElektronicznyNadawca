...
class DeklaracjaCelna2Type extends AbstractStructBase
{
     ...
     * - maxOccurs: 5
     * - minOccurs: 1
     * @var \WellCommerce\ElektronicznyNadawca\StructType\SzczegolyZawartosciPrzesylkiZagranicznejType[]
     */
     ...
     * - maxOccurs: 5
     * - minOccurs: 0
     * @var \WellCommerce\ElektronicznyNadawca\StructType\DokumentyTowarzyszaceType[]
     */
     ...
     * @param \WellCommerce\ElektronicznyNadawca\StructType\SzczegolyZawartosciPrzesylkiZagranicznejType[] $szczegolyZawartosciPrzesylki
     * @param \WellCommerce\ElektronicznyNadawca\StructType\DokumentyTowarzyszaceType[] $dokumentyTowarzyszace
    ...
    /**
     * Set rodzaj value
     * @uses \WellCommerce\ElektronicznyNadawca\EnumType\DeklaracaCelnaRodzajEnum::valueIsValid()
     * @uses \WellCommerce\ElektronicznyNadawca\EnumType\DeklaracaCelnaRodzajEnum::getValidValues()
     * @throws InvalidArgumentException
     * @param string $rodzaj
     * @return \WellCommerce\ElektronicznyNadawca\StructType\DeklaracjaCelna2Type
     */
    public function setRodzaj(string $rodzaj): self
    {
        // validation for constraint: enumeration
        if (!\WellCommerce\ElektronicznyNadawca\EnumType\DeklaracaCelnaRodzajEnum::valueIsValid($rodzaj)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \WellCommerce\ElektronicznyNadawca\EnumType\DeklaracaCelnaRodzajEnum', is_array($rodzaj) ? implode(', ', $rodzaj) : var_export($rodzaj, true), implode(', ', \WellCommerce\ElektronicznyNadawca\EnumType\DeklaracaCelnaRodzajEnum::getValidValues())), __LINE__);
        }
        $this->rodzaj = $rodzaj;
     ...
     * @uses \WellCommerce\ElektronicznyNadawca\EnumType\ZawartoscPrzesylkiZagranicznejEnum::valueIsValid()
     * @uses \WellCommerce\ElektronicznyNadawca\EnumType\ZawartoscPrzesylkiZagranicznejEnum::getValidValues()
     * @throws InvalidArgumentException
     * @param string $zawartoscPrzesylki
     * @return \WellCommerce\ElektronicznyNadawca\StructType\DeklaracjaCelna2Type
     */
    ...
    {
        // validation for constraint: enumeration
        if (!\WellCommerce\ElektronicznyNadawca\EnumType\ZawartoscPrzesylkiZagranicznejEnum::valueIsValid($zawartoscPrzesylki)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \WellCommerce\ElektronicznyNadawca\EnumType\ZawartoscPrzesylkiZagranicznejEnum', is_array($zawartoscPrzesylki) ? implode(', ', $zawartoscPrzesylki) : var_export($zawartoscPrzesylki, true), implode(', ', \WellCommerce\ElektronicznyNadawca\EnumType\ZawartoscPrzesylkiZagranicznejEnum::getValidValues())), __LINE__);
        }
     ...
     * @return \WellCommerce\ElektronicznyNadawca\StructType\SzczegolyZawartosciPrzesylkiZagranicznejType[]
     */
     ...
     */
    public static function validateSzczegolyZawartosciPrzesylkiForArrayConstraintsFromSetSzczegolyZawartosciPrzesylki(?array $values = []): string
    {
        if (!is_array($values)) {
            return '';
        }
        ...
        foreach ($values as $deklaracjaCelna2TypeSzczegolyZawartosciPrzesylkiItem) {
            ...
            if (!$deklaracjaCelna2TypeSzczegolyZawartosciPrzesylkiItem instanceof \WellCommerce\ElektronicznyNadawca\StructType\SzczegolyZawartosciPrzesylkiZagranicznejType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The szczegolyZawartosciPrzesylki property can only contain items of type \WellCommerce\ElektronicznyNadawca\StructType\SzczegolyZawartosciPrzesylkiZagranicznejType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \WellCommerce\ElektronicznyNadawca\StructType\SzczegolyZawartosciPrzesylkiZagranicznejType $item
     * @return \WellCommerce\ElektronicznyNadawca\StructType\DeklaracjaCelna2Type
     */
    public function addToSzczegolyZawartosciPrzesylki(\WellCommerce\ElektronicznyNadawca\StructType\SzczegolyZawartosciPrzesylkiZagranicznejType $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \WellCommerce\ElektronicznyNadawca\StructType\SzczegolyZawartosciPrzesylkiZagranicznejType) {
            throw new InvalidArgumentException(sprintf('The szczegolyZawartosciPrzesylki property can only contain items of type \WellCommerce\ElektronicznyNadawca\StructType\SzczegolyZawartosciPrzesylkiZagranicznejType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(5)
     ...
     * @return \WellCommerce\ElektronicznyNadawca\StructType\DokumentyTowarzyszaceType[]
     */
     ...
     * @return string A non-empty message if the values does not match the validation rules
     */
    public static function validateDokumentyTowarzyszaceForArrayConstraintsFromSetDokumentyTowarzyszace(?array $values = []): string
    {
        if (!is_array($values)) {
            return '';
        }
        ...
        foreach ($values as $deklaracjaCelna2TypeDokumentyTowarzyszaceItem) {
            // validation for constraint: itemType
            if (!$deklaracjaCelna2TypeDokumentyTowarzyszaceItem instanceof \WellCommerce\ElektronicznyNadawca\StructType\DokumentyTowarzyszaceType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The dokumentyTowarzyszace property can only contain items of type \WellCommerce\ElektronicznyNadawca\StructType\DokumentyTowarzyszaceType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \WellCommerce\ElektronicznyNadawca\StructType\DokumentyTowarzyszaceType $item
     * @return \WellCommerce\ElektronicznyNadawca\StructType\DeklaracjaCelna2Type
     */
    public function addToDokumentyTowarzyszace(\WellCommerce\ElektronicznyNadawca\StructType\DokumentyTowarzyszaceType $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \WellCommerce\ElektronicznyNadawca\StructType\DokumentyTowarzyszaceType) {
            throw new InvalidArgumentException(sprintf('The dokumentyTowarzyszace property can only contain items of type \WellCommerce\ElektronicznyNadawca\StructType\DokumentyTowarzyszaceType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(5)
