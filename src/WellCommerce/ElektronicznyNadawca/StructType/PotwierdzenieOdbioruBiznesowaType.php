...
class PotwierdzenieOdbioruBiznesowaType extends AbstractStructBase
{
    ...
    /**
     * Set sposob value
     * @uses \WellCommerce\ElektronicznyNadawca\EnumType\SposobPrzekazaniaPotwierdzeniaBiznesowaType::valueIsValid()
     * @uses \WellCommerce\ElektronicznyNadawca\EnumType\SposobPrzekazaniaPotwierdzeniaBiznesowaType::getValidValues()
     * @throws InvalidArgumentException
     * @param string $sposob
     * @return \WellCommerce\ElektronicznyNadawca\StructType\PotwierdzenieOdbioruBiznesowaType
     */
    public function setSposob(string $sposob): self
    {
        // validation for constraint: enumeration
        if (!\WellCommerce\ElektronicznyNadawca\EnumType\SposobPrzekazaniaPotwierdzeniaBiznesowaType::valueIsValid($sposob)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \WellCommerce\ElektronicznyNadawca\EnumType\SposobPrzekazaniaPotwierdzeniaBiznesowaType', is_array($sposob) ? implode(', ', $sposob) : var_export($sposob, true), implode(', ', \WellCommerce\ElektronicznyNadawca\EnumType\SposobPrzekazaniaPotwierdzeniaBiznesowaType::getValidValues())), __LINE__);
        }
        $this->sposob = $sposob;
