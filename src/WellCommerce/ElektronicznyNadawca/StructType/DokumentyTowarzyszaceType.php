...
class DokumentyTowarzyszaceType extends AbstractStructBase
{
    ...
    /**
     * Set rodzaj value
     * @uses \WellCommerce\ElektronicznyNadawca\EnumType\DokumentTowarzyszacyRodzajEnum::valueIsValid()
     * @uses \WellCommerce\ElektronicznyNadawca\EnumType\DokumentTowarzyszacyRodzajEnum::getValidValues()
    ...
    public function setRodzaj(?string $rodzaj = null): self
    {
        ...
        if (!\WellCommerce\ElektronicznyNadawca\EnumType\DokumentTowarzyszacyRodzajEnum::valueIsValid($rodzaj)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \WellCommerce\ElektronicznyNadawca\EnumType\DokumentTowarzyszacyRodzajEnum', is_array($rodzaj) ? implode(', ', $rodzaj) : var_export($rodzaj, true), implode(', ', \WellCommerce\ElektronicznyNadawca\EnumType\DokumentTowarzyszacyRodzajEnum::getValidValues())), __LINE__);
        }
        $this->rodzaj = $rodzaj;
