...
class CustomsDeclarationType extends AbstractStructBase
{
     ...
     * - maxOccurs: unbounded
     * - minOccurs: 1
     * @var \WellCommerce\ElektronicznyNadawca\StructType\ShipmentContentsDetailsType[]
     */
     ...
     * - maxOccurs: unbounded
     * - minOccurs: 0
     * @var \WellCommerce\ElektronicznyNadawca\StructType\AccompanyingDocumentsType[]
     */
     ...
     * @param \WellCommerce\ElektronicznyNadawca\StructType\ShipmentContentsDetailsType[] $shipmentContentsDetails
     * @param string $type
     * @param \WellCommerce\ElektronicznyNadawca\StructType\AccompanyingDocumentsType[] $accompanyingDocuments
    ...
    /**
     * Set content value
     * @uses \WellCommerce\ElektronicznyNadawca\EnumType\CustomsDeclarationContentEnum::valueIsValid()
     * @uses \WellCommerce\ElektronicznyNadawca\EnumType\CustomsDeclarationContentEnum::getValidValues()
     * @throws InvalidArgumentException
     * @param string $content
     * @return \WellCommerce\ElektronicznyNadawca\StructType\CustomsDeclarationType
     */
    public function setContent(string $content): self
    {
        // validation for constraint: enumeration
        if (!\WellCommerce\ElektronicznyNadawca\EnumType\CustomsDeclarationContentEnum::valueIsValid($content)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \WellCommerce\ElektronicznyNadawca\EnumType\CustomsDeclarationContentEnum', is_array($content) ? implode(', ', $content) : var_export($content, true), implode(', ', \WellCommerce\ElektronicznyNadawca\EnumType\CustomsDeclarationContentEnum::getValidValues())), __LINE__);
        }
     ...
     * @return \WellCommerce\ElektronicznyNadawca\StructType\ShipmentContentsDetailsType[]
     */
     ...
     */
    public static function validateShipmentContentsDetailsForArrayConstraintsFromSetShipmentContentsDetails(?array $values = []): string
    {
        if (!is_array($values)) {
            return '';
        }
        ...
        foreach ($values as $customsDeclarationTypeShipmentContentsDetailsItem) {
            ...
            if (!$customsDeclarationTypeShipmentContentsDetailsItem instanceof \WellCommerce\ElektronicznyNadawca\StructType\ShipmentContentsDetailsType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The shipmentContentsDetails property can only contain items of type \WellCommerce\ElektronicznyNadawca\StructType\ShipmentContentsDetailsType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \WellCommerce\ElektronicznyNadawca\StructType\ShipmentContentsDetailsType $item
     * @return \WellCommerce\ElektronicznyNadawca\StructType\CustomsDeclarationType
     */
    public function addToShipmentContentsDetails(\WellCommerce\ElektronicznyNadawca\StructType\ShipmentContentsDetailsType $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \WellCommerce\ElektronicznyNadawca\StructType\ShipmentContentsDetailsType) {
            throw new InvalidArgumentException(sprintf('The shipmentContentsDetails property can only contain items of type \WellCommerce\ElektronicznyNadawca\StructType\ShipmentContentsDetailsType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
    ...
    /**
     * Set type value
     * @uses \WellCommerce\ElektronicznyNadawca\EnumType\CustomsDeclarationTypeEnum::valueIsValid()
     * @uses \WellCommerce\ElektronicznyNadawca\EnumType\CustomsDeclarationTypeEnum::getValidValues()
     * @throws InvalidArgumentException
     * @param string $type
     * @return \WellCommerce\ElektronicznyNadawca\StructType\CustomsDeclarationType
     */
    ...
    {
        // validation for constraint: enumeration
        if (!\WellCommerce\ElektronicznyNadawca\EnumType\CustomsDeclarationTypeEnum::valueIsValid($type)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \WellCommerce\ElektronicznyNadawca\EnumType\CustomsDeclarationTypeEnum', is_array($type) ? implode(', ', $type) : var_export($type, true), implode(', ', \WellCommerce\ElektronicznyNadawca\EnumType\CustomsDeclarationTypeEnum::getValidValues())), __LINE__);
        }
        $this->type = $type;
     ...
     * @return string A non-empty message if the values does not match the validation rules
     */
    public static function validateAccompanyingDocumentsForArrayConstraintsFromSetAccompanyingDocuments(?array $values = []): string
    {
        if (!is_array($values)) {
            return '';
        }
        ...
        foreach ($values as $customsDeclarationTypeAccompanyingDocumentsItem) {
            // validation for constraint: itemType
            if (!$customsDeclarationTypeAccompanyingDocumentsItem instanceof \WellCommerce\ElektronicznyNadawca\StructType\AccompanyingDocumentsType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The accompanyingDocuments property can only contain items of type \WellCommerce\ElektronicznyNadawca\StructType\AccompanyingDocumentsType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \WellCommerce\ElektronicznyNadawca\StructType\AccompanyingDocumentsType $item
     * @return \WellCommerce\ElektronicznyNadawca\StructType\CustomsDeclarationType
     */
    public function addToAccompanyingDocuments(\WellCommerce\ElektronicznyNadawca\StructType\AccompanyingDocumentsType $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \WellCommerce\ElektronicznyNadawca\StructType\AccompanyingDocumentsType) {
            throw new InvalidArgumentException(sprintf('The accompanyingDocuments property can only contain items of type \WellCommerce\ElektronicznyNadawca\StructType\AccompanyingDocumentsType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
