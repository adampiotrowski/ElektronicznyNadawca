...
class DaneSentType extends AbstractStructBase
{
     ...
     */
    public function validateKodCNForChoiceConstraintsFromSetKodCN($value): string
    {
        $message = '';
        if (is_null($value)) {
            return $message;
     ...
     * @return string A non-empty message if the values does not match the validation rules
     */
    public function validateKodPKWiUForChoiceConstraintsFromSetKodPKWiU($value): string
    {
        $message = '';
        if (is_null($value)) {
            return $message;
