...
class DeklaracjaCelnaType extends AbstractStructBase
{
     ...
     * - maxOccurs: 5
     * - minOccurs: 1
     * @var \WellCommerce\ElektronicznyNadawca\StructType\SzczegolyDeklaracjiCelnejType[]
     */
     ...
     * @param \WellCommerce\ElektronicznyNadawca\StructType\SzczegolyDeklaracjiCelnejType[] $szczegoly
    ...
    /**
     * Get szczegoly value
     * @return \WellCommerce\ElektronicznyNadawca\StructType\SzczegolyDeklaracjiCelnejType[]
     */
     ...
     */
    public static function validateSzczegolyForArrayConstraintsFromSetSzczegoly(?array $values = []): string
    {
        if (!is_array($values)) {
            return '';
        }
        ...
        foreach ($values as $deklaracjaCelnaTypeSzczegolyItem) {
            ...
            if (!$deklaracjaCelnaTypeSzczegolyItem instanceof \WellCommerce\ElektronicznyNadawca\StructType\SzczegolyDeklaracjiCelnejType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The szczegoly property can only contain items of type \WellCommerce\ElektronicznyNadawca\StructType\SzczegolyDeklaracjiCelnejType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \WellCommerce\ElektronicznyNadawca\StructType\SzczegolyDeklaracjiCelnejType[] $szczegoly
     * @return \WellCommerce\ElektronicznyNadawca\StructType\DeklaracjaCelnaType
     */
     ...
     * @throws InvalidArgumentException
     * @param \WellCommerce\ElektronicznyNadawca\StructType\SzczegolyDeklaracjiCelnejType $item
     * @return \WellCommerce\ElektronicznyNadawca\StructType\DeklaracjaCelnaType
     */
    public function addToSzczegoly(\WellCommerce\ElektronicznyNadawca\StructType\SzczegolyDeklaracjiCelnejType $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \WellCommerce\ElektronicznyNadawca\StructType\SzczegolyDeklaracjiCelnejType) {
            throw new InvalidArgumentException(sprintf('The szczegoly property can only contain items of type \WellCommerce\ElektronicznyNadawca\StructType\SzczegolyDeklaracjiCelnejType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(5)
