...
class ZwrotType extends AbstractStructBase
{
     ...
     * @param int $zwrotPoLiczbieDni
     * @return \WellCommerce\ElektronicznyNadawca\StructType\ZwrotType
     */
     ...
     * @uses \WellCommerce\ElektronicznyNadawca\EnumType\SposobZwrotuType::valueIsValid()
     * @uses \WellCommerce\ElektronicznyNadawca\EnumType\SposobZwrotuType::getValidValues()
    ...
    public function setSposobZwrotu(?string $sposobZwrotu = null): self
    {
        ...
        if (!\WellCommerce\ElektronicznyNadawca\EnumType\SposobZwrotuType::valueIsValid($sposobZwrotu)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \WellCommerce\ElektronicznyNadawca\EnumType\SposobZwrotuType', is_array($sposobZwrotu) ? implode(', ', $sposobZwrotu) : var_export($sposobZwrotu, true), implode(', ', \WellCommerce\ElektronicznyNadawca\EnumType\SposobZwrotuType::getValidValues())), __LINE__);
        }
