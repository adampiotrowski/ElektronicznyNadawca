...
class GetEnvelopeContentFullResponse extends AbstractStructBase
{
     ...
     * - maxOccurs: unbounded
     * - minOccurs: 0
     * @var \WellCommerce\ElektronicznyNadawca\StructType\PrzesylkaFullType[]
     */
     ...
     * @param \WellCommerce\ElektronicznyNadawca\StructType\PrzesylkaFullType[] $przesylka
     */
    ...
    public static function validatePrzesylkaForArrayConstraintsFromSetPrzesylka(?array $values = []): string
    {
        ...
        foreach ($values as $getEnvelopeContentFullResponsePrzesylkaItem) {
            ...
            if (!$getEnvelopeContentFullResponsePrzesylkaItem instanceof \WellCommerce\ElektronicznyNadawca\StructType\PrzesylkaFullType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The przesylka property can only contain items of type \WellCommerce\ElektronicznyNadawca\StructType\PrzesylkaFullType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \WellCommerce\ElektronicznyNadawca\StructType\PrzesylkaFullType[] $przesylka
     * @return \WellCommerce\ElektronicznyNadawca\StructType\GetEnvelopeContentFullResponse
     */
     ...
     * @throws InvalidArgumentException
     * @param \WellCommerce\ElektronicznyNadawca\StructType\PrzesylkaFullType $item
     * @return \WellCommerce\ElektronicznyNadawca\StructType\GetEnvelopeContentFullResponse
     */
    public function addToPrzesylka(\WellCommerce\ElektronicznyNadawca\StructType\PrzesylkaFullType $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \WellCommerce\ElektronicznyNadawca\StructType\PrzesylkaFullType) {
            throw new InvalidArgumentException(sprintf('The przesylka property can only contain items of type \WellCommerce\ElektronicznyNadawca\StructType\PrzesylkaFullType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        $this->przesylka[] = $item;
