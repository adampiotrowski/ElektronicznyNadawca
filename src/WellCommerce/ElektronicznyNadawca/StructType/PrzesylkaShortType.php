...
class PrzesylkaShortType extends AbstractStructBase
{
     ...
     */
    public static function validateCzynnosciUpustoweForArrayConstraintsFromSetCzynnosciUpustowe(?array $values = []): string
    {
        if (!is_array($values)) {
            return '';
        }
        ...
        foreach ($values as $przesylkaShortTypeCzynnosciUpustoweItem) {
            ...
            if (!\WellCommerce\ElektronicznyNadawca\EnumType\CzynnoscUpustowaType::valueIsValid($przesylkaShortTypeCzynnosciUpustoweItem)) {
    ...
    /**
     * Set status value
     * @uses \WellCommerce\ElektronicznyNadawca\EnumType\StatusType::valueIsValid()
     * @uses \WellCommerce\ElektronicznyNadawca\EnumType\StatusType::getValidValues()
     * @throws InvalidArgumentException
     * @param string $status
     * @return \WellCommerce\ElektronicznyNadawca\StructType\PrzesylkaShortType
     */
    ...
    {
        // validation for constraint: enumeration
        if (!\WellCommerce\ElektronicznyNadawca\EnumType\StatusType::valueIsValid($status)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \WellCommerce\ElektronicznyNadawca\EnumType\StatusType', is_array($status) ? implode(', ', $status) : var_export($status, true), implode(', ', \WellCommerce\ElektronicznyNadawca\EnumType\StatusType::getValidValues())), __LINE__);
        }
        $this->status = $status;
