...
class PotwierdzenieOdbioruKurierskaType extends AbstractStructBase
{
     ...
     * Set ilosc value
     * @param int $ilosc
     * @return \WellCommerce\ElektronicznyNadawca\StructType\PotwierdzenieOdbioruKurierskaType
     */
    ...
    /**
     * Set sposob value
     * @uses \WellCommerce\ElektronicznyNadawca\EnumType\SposobPrzekazaniaPotwierdzeniaOdbioruKurierskaType::valueIsValid()
     * @uses \WellCommerce\ElektronicznyNadawca\EnumType\SposobPrzekazaniaPotwierdzeniaOdbioruKurierskaType::getValidValues()
    ...
    public function setSposob(?string $sposob = null): self
    {
        ...
        if (!\WellCommerce\ElektronicznyNadawca\EnumType\SposobPrzekazaniaPotwierdzeniaOdbioruKurierskaType::valueIsValid($sposob)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \WellCommerce\ElektronicznyNadawca\EnumType\SposobPrzekazaniaPotwierdzeniaOdbioruKurierskaType', is_array($sposob) ? implode(', ', $sposob) : var_export($sposob, true), implode(', ', \WellCommerce\ElektronicznyNadawca\EnumType\SposobPrzekazaniaPotwierdzeniaOdbioruKurierskaType::getValidValues())), __LINE__);
        }
        $this->sposob = $sposob;
