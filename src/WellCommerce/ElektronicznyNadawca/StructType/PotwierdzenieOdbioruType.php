...
class PotwierdzenieOdbioruType extends AbstractStructBase
{
     ...
     * Set ilosc value
     * @param int $ilosc
     * @return \WellCommerce\ElektronicznyNadawca\StructType\PotwierdzenieOdbioruType
     */
    ...
    /**
     * Set sposob value
     * @uses \WellCommerce\ElektronicznyNadawca\EnumType\SposobPrzekazaniaPotwierdzeniaOdbioruType::valueIsValid()
     * @uses \WellCommerce\ElektronicznyNadawca\EnumType\SposobPrzekazaniaPotwierdzeniaOdbioruType::getValidValues()
    ...
    public function setSposob(?string $sposob = null): self
    {
        ...
        if (!\WellCommerce\ElektronicznyNadawca\EnumType\SposobPrzekazaniaPotwierdzeniaOdbioruType::valueIsValid($sposob)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \WellCommerce\ElektronicznyNadawca\EnumType\SposobPrzekazaniaPotwierdzeniaOdbioruType', is_array($sposob) ? implode(', ', $sposob) : var_export($sposob, true), implode(', ', \WellCommerce\ElektronicznyNadawca\EnumType\SposobPrzekazaniaPotwierdzeniaOdbioruType::getValidValues())), __LINE__);
        }
        $this->sposob = $sposob;
