...
class PowodReklamacjiType extends AbstractStructBase
{
     ...
     * - maxOccurs: unbounded
     * - minOccurs: 1
     * @var \WellCommerce\ElektronicznyNadawca\StructType\PowodSzczegolowyType[]
     */
     ...
     * @param \WellCommerce\ElektronicznyNadawca\StructType\PowodSzczegolowyType[] $powodSzczegolowy
     ...
     * @return \WellCommerce\ElektronicznyNadawca\StructType\PowodSzczegolowyType[]
     */
     ...
     */
    public static function validatePowodSzczegolowyForArrayConstraintsFromSetPowodSzczegolowy(?array $values = []): string
    {
        if (!is_array($values)) {
            return '';
        }
        ...
        foreach ($values as $powodReklamacjiTypePowodSzczegolowyItem) {
            ...
            if (!$powodReklamacjiTypePowodSzczegolowyItem instanceof \WellCommerce\ElektronicznyNadawca\StructType\PowodSzczegolowyType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The powodSzczegolowy property can only contain items of type \WellCommerce\ElektronicznyNadawca\StructType\PowodSzczegolowyType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \WellCommerce\ElektronicznyNadawca\StructType\PowodSzczegolowyType $item
     * @return \WellCommerce\ElektronicznyNadawca\StructType\PowodReklamacjiType
     */
    public function addToPowodSzczegolowy(\WellCommerce\ElektronicznyNadawca\StructType\PowodSzczegolowyType $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \WellCommerce\ElektronicznyNadawca\StructType\PowodSzczegolowyType) {
            throw new InvalidArgumentException(sprintf('The powodSzczegolowy property can only contain items of type \WellCommerce\ElektronicznyNadawca\StructType\PowodSzczegolowyType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
