...
class Pocztex2021NaDzisType extends Pocztex2021Type
{
     ...
     * - maxOccurs: unbounded
     * - minOccurs: 0
     * @var \WellCommerce\ElektronicznyNadawca\StructType\SubPocztex2021NaDzisType[]
     */
     ...
     * @param \WellCommerce\ElektronicznyNadawca\StructType\SubPocztex2021NaDzisType[] $subPrzesylka
     * @param int $odleglosc
     * @param string $obszar
     */
     ...
     * @return \WellCommerce\ElektronicznyNadawca\StructType\SubPocztex2021NaDzisType[]
     */
    ...
    public static function validateSubPrzesylkaForArrayConstraintsFromSetSubPrzesylka(?array $values = []): string
    {
        ...
        foreach ($values as $pocztex2021NaDzisTypeSubPrzesylkaItem) {
            ...
            if (!$pocztex2021NaDzisTypeSubPrzesylkaItem instanceof \WellCommerce\ElektronicznyNadawca\StructType\SubPocztex2021NaDzisType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The subPrzesylka property can only contain items of type \WellCommerce\ElektronicznyNadawca\StructType\SubPocztex2021NaDzisType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \WellCommerce\ElektronicznyNadawca\StructType\SubPocztex2021NaDzisType[] $subPrzesylka
     * @return \WellCommerce\ElektronicznyNadawca\StructType\Pocztex2021NaDzisType
     */
     ...
     * @throws InvalidArgumentException
     * @param \WellCommerce\ElektronicznyNadawca\StructType\SubPocztex2021NaDzisType $item
     * @return \WellCommerce\ElektronicznyNadawca\StructType\Pocztex2021NaDzisType
     */
    public function addToSubPrzesylka(\WellCommerce\ElektronicznyNadawca\StructType\SubPocztex2021NaDzisType $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \WellCommerce\ElektronicznyNadawca\StructType\SubPocztex2021NaDzisType) {
            throw new InvalidArgumentException(sprintf('The subPrzesylka property can only contain items of type \WellCommerce\ElektronicznyNadawca\StructType\SubPocztex2021NaDzisType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
    ...
    /**
     * Set obszar value
     * @uses \WellCommerce\ElektronicznyNadawca\EnumType\ObszarType::valueIsValid()
     * @uses \WellCommerce\ElektronicznyNadawca\EnumType\ObszarType::getValidValues()
     * @throws InvalidArgumentException
     * @param string $obszar
     * @return \WellCommerce\ElektronicznyNadawca\StructType\Pocztex2021NaDzisType
     */
    public function setObszar(?string $obszar = null): self
    {
        ...
        if (!\WellCommerce\ElektronicznyNadawca\EnumType\ObszarType::valueIsValid($obszar)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \WellCommerce\ElektronicznyNadawca\EnumType\ObszarType', is_array($obszar) ? implode(', ', $obszar) : var_export($obszar, true), implode(', ', \WellCommerce\ElektronicznyNadawca\EnumType\ObszarType::getValidValues())), __LINE__);
        }
        $this->obszar = $obszar;
